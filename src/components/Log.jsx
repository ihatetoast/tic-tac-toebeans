...
const SASSY_COMMENTARY ={
  ...
  gameOverTie:["It's a tie game. Pink and Black rule together.", "Like a flamingo, pink and black exist in harmony.", "It night not be a winner winner chicken dinner, but it definitely is a tie tie pizza pie!"]
}

const initRandom = Math.random();
const Log = ({activePlayer, turns, result, handleClick}) => {
...
function getMessage(result){
  ...
  if(result === 'tie'){
    return getRandomCommentary(SASSY_COMMENTARY['gameOverTie']);
  }
}
...
return (
  <div className="log">
    ...
    {result && <p>{"Game over! " + getMessage(result)}</p>}  
    {result && <button className="reset-btn" onClick={handleClick}>Reset</button>}
  </div>
)
}
